...
public final class WebSocketClient {
    ...
    private let payloadSerializer: PayloadSerializerProtocol                        // Сериализатор полезной нагрузки
    ...
    private let delegate: WebSocketTaskDelegate                                     // Делаг
    ...
    public init(url: URL, session: URLSessionProtocol? = nil) {
        self.url = url
        self.delegate = WebSocketTaskDelegate()
        self.payloadSerializer = PayloadSerializer()
        self.session = session ?? SessionWrapper(session: URLSession(configuration: .default, delegate: delegate, delegateQueue: nil))
    }
