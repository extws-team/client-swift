...
final class WebSocketClientTests: XCTestCase {
   ...
   private struct TestPayload: PayloadData {
    ...
    func testMessageHandling() async throws {
        ...
        let payload = TestPayload(value: Constants.receivedPayloadValue)

        client.on(Constants.messageEvent) { data in
            do {
                let decoded: Payload<TestPayload> = try PayloadSerializer().parse(data)
                XCTAssertEqual(decoded.data?.value, Constants.receivedPayloadValue)
                expectation.fulfill()
            } catch {
        ...
        let testData = try PayloadSerializer().build(
            type: .message,
            event: Constants.testPayloadValue,
            data: payload
        )
        mockTask.receiveCompletion?(.success(.data(testData)))
    ...
    func testStringMessageHandling() async throws {
        // Arrange
        let expectation = expectation(description: Constants.handledStringMessageExpectation)
        let payload = TestPayload(value: Constants.stringPayloadValue)

        client.on(Constants.messageEvent) { data in
            do {
                let decoded: Payload<TestPayload> = try PayloadSerializer().parse(data)
                XCTAssertEqual(decoded.data?.value, Constants.stringPayloadValue)
                expectation.fulfill()
            } catch {
                XCTFail(Constants.decodingFailedExpectation)
            }
        }
        ...
        let testData = try PayloadSerializer().build(
            type: .message,
            event: Constants.eventPayloadValue,
            data: payload
        )

        let json = String(data: testData, encoding: .utf8)!
        mockTask.receiveCompletion?(.success(.string(json)))
